...
class RegisterViewModel (val repo: LoginRepository, val app : Application) : ViewModel() {
    ...
    fun submitButton(){
        ...
        if(!registerForm.value!!.isValid()) {
            ...
            runBlocking {
                Timber.d("FORM IS VALID MAKING REQUEST")
                var reponse = repo.register(registerForm.value!!)
                if(reponse != null){
